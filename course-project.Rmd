...
This project analyses data from accelerometers on the belt, forearm, arm, and dumbell of 6 participants. They were asked to perform barbell lifts correctly and incorrectly in 5 different ways. The data for this project is taken from the homepage (http://groupware.les.inf.puc-rio.br/har)[http://groupware.les.inf.puc-rio.br/har]. This website also contains further information on the data set. 
...
```{r, cache=TRUE}
modelFit_randomForest <- suppressMessages(train(training$classe ~ ., method="rf", trControl=trainControl(method = "cv", number = 4), data=training))
